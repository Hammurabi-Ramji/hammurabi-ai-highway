...
mod tests {
    ...
    use crate::pipeline::{Payload, PipelineContext, Artifact, bsm, Stage};
    ...
    async fn test_bsm_persists_artifacts() {
        ...
        let mut stmt = conn.prepare("SELECT artifact FROM code_registry WHERE intent_hash = ?1").unwrap();
        let artifact: String = stmt.query_row(["0123456789abcdef"], |row| row.get(0)).unwrap();
