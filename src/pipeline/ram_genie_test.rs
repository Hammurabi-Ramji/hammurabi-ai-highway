...
mod tests {
    ...
    use crate::pipeline::{Payload, PipelineContext, ram_genie, Stage};
    ...
    async fn test_onchain_requirement_detection() {
        ...
        assert!(result.requirements.contains(&"on-chain execution required".to_string()));
    }
    ...
    async fn test_frontend_requirement_detection() {
        ...
        assert!(result.requirements.contains(&"frontend surface required".to_string()));
    }
    ...
    async fn test_auth_requirement_detection() {
        ...
        assert!(result.requirements.contains(&"authentication required".to_string()));
    }
    ...
    async fn test_generic_build_request() {
        ...
        assert!(result.requirements.contains(&"generic build request".to_string()));
    }
    ...
    async fn test_case_insensitive_parsing() {
        ...
        for intent in intents {
            ...
            assert!(result.requirements.contains(&"on-chain execution required".to_string()),
                    "Intent '{}' should detect on-chain requirement", intent);
        }
    }
}
